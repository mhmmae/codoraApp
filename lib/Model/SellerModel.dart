...
class SellerModel {
  final String uid; // معرف المستخدم من FirebaseAuth، سيكون هو ID المستند في مجموعة البائعين
  ...
  // الفئة وساعات العمل
  final String shopCategory; // الفئة الرئيسية للمحل (مثل: إلكترونيات، ملابس)
  final Map<String, dynamic> workingHours; // هيكل Map لتخزين ساعات العمل لكل يوم

  // معلومات إضافية اختيارية
  final String? commercialRegistrationNumber;
  final String? websiteUrl;
  final Map<String, String>? socialMediaLinks; // مثال: {'facebook': 'url', 'instagram': 'url'}

  // حالات وحالة التحقق من قبل المشرف (إذا كان لديك نظام موافقة للبائعين)
  final bool isProfileComplete;      // هل أكمل البائع ملفه (عادةً ما يكون true بعد هذه العملية)
  final bool isApprovedByAdmin;      // هل تمت الموافقة على البائع من قبل مشرف التطبيق
  ...
  // الحقول الجديدة
  final String? sellerType; // نوع البائع: 'wholesale', 'retail', أو null إذا لم يحدد بعد
  ...
  SellerModel({
    ...
    this.isProfileComplete = true, // يفترض أنه true عند إنشاء هذا النموذج بعد التسجيل
    this.isApprovedByAdmin = false, // يبدأ غير موافق عليه (إذا كان هناك نظام موافقة)
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'createdAt': createdAt, // أو FieldValue.serverTimestamp() إذا كان الإنشاء من جانب الخادم أو في معاملة
      'updatedAt': FieldValue.serverTimestamp(), // يتم تحديثه تلقائيًا مع كل عملية set/update
    };
  }
  ...
  factory SellerModel.fromMap(Map<String, dynamic> map, String documentId) {
    ...
    return SellerModel(
      ...
      location: map['location'] as GeoPoint? ?? const GeoPoint(0,0), // قيمة افتراضية إذا كان null
      shopAddressText: map['shopAddressText'] as String?,
      shopCategory: map['shopCategory'] as String? ?? 'أخرى', // قيمة افتراضية
      ...
      registrationCompleted: map['registrationCompleted'] as bool? ?? false, // إضافة هنا
      createdAt: map['createdAt'] as Timestamp? ?? Timestamp.now(), // قيمة افتراضية
  ...
  SellerModel copyWith({
    ...
    return SellerModel(
        ...
        registrationCompleted: registrationCompleted ?? this.registrationCompleted, // إضافة هنا
        createdAt: createdAt ?? this.createdAt,
        updatedAt: updatedAt // يمكن تمرير null إذا كان سيتحدث بواسطة serverTimestamp
    );
  }
