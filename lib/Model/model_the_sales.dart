...
class ModelTheSales{
  ...
  factory ModelTheSales.fromMap(Map<String,dynamic>map){
    return ModelTheSales(DeliveryUid: map['DeliveryUid']??'', appName: map['appName']??'', longitude: map['longitude']??'', latitude: map['latitude']??'',
        timeOrder: map['timeOrder'] is Timestamp ? (map['timeOrder'] as Timestamp).toDate() : (map['timeOrder'] is String ? (DateTime.tryParse(map['timeOrder']) ?? DateTime.now()) : DateTime.now()),
// ونفس الشيء لـ timeDeliveryOrder و timeOrderDone,
