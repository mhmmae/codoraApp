...
// تحميل المفضلة ونوع العرض
  ...
  Future<void> shareCodeGroup(Map<String, dynamic> item, BuildContext context) async { // أبقينا context حاليًا لكن قد لا نحتاجه
    ...
    print("Attempting to share: $shareText"); // طباعة النص للمساعدة في التصحيح
    ...
    try {
      ...
      print("Sharing seems successful.");

    } catch (e, stackTrace) { // <-- إضافة stackTrace هنا
      print("Share Error: $e"); // طباعة الخطأ المحدد
      print("Share StackTrace: $stackTrace"); // <-- طباعة تتبع المكدس
      Get.snackbar(
          "خطأ في المشاركة",
  ...
  void navigateToDetail(Map<String, dynamic> item) {
    print(">>> Navigating to detail for: ${item['codeName']}");
    try {
  ...
  Future<void> incrementCopyCount(String codeGroupId) async {
    ...
    try {
      ...
      print("Copy count incremented for $codeGroupId");
    } catch (e, s) {
      print("Error incrementing copy count for $codeGroupId: $e \n $s");
    }
  }
...
// --- إضافة / تحديث تقييم وتعليق (مع جلب الاسم من Usercodora) ---
  Future<bool> addReview({
    ...
    try {
      ...
      print("Error fetching user name from Usercodora: $e\n$s");
    ...
    try {
      ...
      if (existingReviewId != null && existingReviewId.isNotEmpty) {
        ...
        print("Review updated: $existingReviewId");
        ...
        if (existingDocs.docs.isNotEmpty) {
          ...
          print("Review already existed, updated instead: $docIdToUpdate");
          ...
          print("Review added: ${newReviewRef.id}");
          Get.snackbar("شكراً لك", "تم إضافة تقييمك بنجاح.", /*...*/);
        }
      }

      // **تذكير:** قم بتنفيذ تحديث المتوسط باستخدام Cloud Function!
      print("Reminder: Update average rating via Cloud Function is recommended.");

      return true; // نجاح

    } catch (e, s) {
      print("Error saving review for $codeGroupId: $e \n $s");
  ...
  Future<bool> reportCodeGroup({
    ...
    try {
      ...
      print("Report submitted for $codeGroupId by ${currentUser.uid}");
      return true; // نجاح

    } catch (e, s) {
      print("Error submitting report for $codeGroupId: $e \n $s");
  ...
  void _scrollListener() {
    ...
    if (scrollController.position.pixels >= scrollController.position.maxScrollExtent - 300 &&
      ...
      print(">>> Scroll Listener: Reached end, loading more...");
