...
class CodeRequestController extends GetxController {
  ...
  Future<void> initializeForCode(String? codeName) async {
    if (codeName == null || codeName.isEmpty) {
      print("CodeRequestController: اسم الكود غير موجود، لا يمكن التهيئة.");
    ...
    print("CodeRequestController: بدء تهيئة لـ $codeName");
    ...
    try {
      ...
      print("CodeRequestController: خطأ عام أثناء التهيئة لـ $codeName: $e\n$s");
      ...
      print("CodeRequestController: انتهت تهيئة $codeName. isLoading = ${isLoadingDurationsAndPrice.value}");
  ...
  Future<void> _fetchPricingData(String codeName) async {
    print("جلب بيانات التسعير لـ $codeName...");
    try {
      ...
      if (docSnapshot.exists && docSnapshot.data() != null) {
        ...
        print("تم العثور على بيانات التسعير: ${pricingData.value}");
      } else {
        pricingData.value = {}; // لا توجد بيانات تسعير لهذا الكود
        print("لم يتم العثور على وثيقة تسعير لـ $codeName.");
      }
    } catch (e, s) {
      print("خطأ أثناء جلب بيانات التسعير لـ $codeName: $e\n$s");
  ...
  Future<void> _fetchAvailableDurations(String codeNameParam) async {
    // --- بدء العملية ---
    print("التحقق من المدد المتاحة (أكواد + سعر) لـ '$codeNameParam'...");
    ...
    try {
      ...
      for (var durationMap in _allDurationOptions) {
        ...
        if (pricingData.value.containsKey(durationValueEn)) {
          ...
          print("السعر موجود للمدة '$durationValueEn'. الآن نبحث عن كود متاح...");
          ...
          print("السعر غير معرف للمدة '$durationValueEn'، تخطي التحقق من الأكواد.");
      ...
      print("انتظار نتائج البحث عن الأكواد لكل المدد...");
      final List<QuerySnapshot?> results = await Future.wait(codeCheckFutures);
      print("تم الحصول على ${results.length} نتيجة بحث.");
      ...
      for (int i = 0; i < results.length; i++) {
        ...
        if (queryResult != null && queryResult.docs.isNotEmpty) {
          ...
          print("✔️ المدة '${durationMap['en']}' متاحة للشراء (يوجد سعر وكود).");
        }
        ...
        // أو يمكنك إضافة طباعة للمدد التي لها سعر ولكن لا أكواد:
          ...
          print("⏳ المدة '${durationMap['en']}' غير متاحة حالياً (يوجد سعر لكن لا توجد أكواد متوفرة).");
        }
      }
...
// نهاية حلقة تحليل النتائج
      ...
      print("حدث خطأ أثناء التحقق من المدد المتاحة لـ $codeNameParam: $e \n$s");
    ...
    if (foundAndPricedDurations.isNotEmpty) {
      ...
      print("تم تحديد '${foundAndPricedDurations.first['ar']}' كمدة افتراضية.");
    } else {
      selectedDurationOption.value = null; // لا يوجد اختيار افتراضي إذا لم تتوفر أي مدة
      print("لا توجد مدد متاحة لتحديد اختيار افتراضي.");
    }
  ...
  void _updateCurrentPrice() {
    ...
    if (selectedEn != null && pricingData.value.containsKey(selectedEn)) {
      ...
      if (priceValue is num) { // التأكد أن السعر هو رقم (int أو double)
        ...
        print("تم تحديد السعر الحالي بـ: ${currentPrice.value} للمدة $selectedEn");
      } else {
        // إذا كانت القيمة المخزنة للسعر ليست رقمًا
        print("قيمة السعر للمدة $selectedEn ليست رقمًا: $priceValue");
        currentPrice.value = null; // تعيين السعر كـ null
      }
    } else {
      // إذا لم يتم اختيار مدة أو لا يوجد سعر للمدة المختارة
      currentPrice.value = null;
      print("لا يوجد سعر أو مدة مختارة. السعر الحالي هو null.");
    }
  }
...
// مرجع للمستند الذي سيتم تحديثه
    ...
    try {
      ...
      print("Searching for available code: Name='$codeName', Duration='$selectedDuration', isRUN=false");
      ...
      if (snapshot.docs.isEmpty) {
        print("No available code found matching criteria outside transaction.");
      ...
      print("Found potential code: ${codeToAssignRef.id}. Starting transaction...");
      ...
      assignedCodeValue = await _firestore.runTransaction<String?>((transaction) async {
        ...
        if (!freshDocSnapshot.exists) {
          print("Code ${codeToAssignRef.id} no longer exists.");
        ...
        if (codeDocData == null || codeDocData['isRUN'] == true || codeDocData['assignedToUserId'] != null) {
          print("Code ${codeToAssignRef.id} was claimed during transaction.");
        ...
        print("Transaction successful: Updated code ${codeToAssignRef.id} for user ${currentUser.uid}");
...
// --- ▼▼▼ *جديد*: جلب بيانات المستخدم لـ الفاتورة ▼▼▼ ---
        ...
        try {
          print("Fetching user data from Usercodora for invoice...");
          ...
          if (userDocSnapshot.exists && userDocSnapshot.data() != null) {
            ...
            print("Usercodora data found: $userData");
            ...
            print("User document NOT found in Usercodora for ${currentUser.uid}");
          }
        } catch(e, s) {
          print("!!! Error fetching user data for invoice (using defaults): $e\n$s");
        ...
        try {
          ...
          print("Sales record created successfully: $saleUuid");
          ...
          print("!!! Error creating sales record for $codeName ($selectedDuration) AFTER successful assignment: $e\n$s");
        ...
        print("Transaction completed but assignedCodeValue is null (unexpected).");
      ...
      print("Firestore Transaction Error: ${e.code} - ${e.message}");
      ...
      print("Generic Error during code request: $e\n$s");
