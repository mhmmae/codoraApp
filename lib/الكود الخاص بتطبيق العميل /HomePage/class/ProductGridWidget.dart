...
class ProductGridWidgetOption extends StatefulWidget {
  ...
  @override
  State<ProductGridWidgetOption> createState() => _ProductGridWidgetOptionState();
}

class _ProductGridWidgetOptionState extends State<ProductGridWidgetOption> {
  final String allItemsFilterKey = 'all_items';
  final bool _isAdmin = FirebaseAuth.instance.currentUser?.email == FirebaseX.EmailOfWnerApp;
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> _buildProductStreamWithRetry({bool skipSort = false}) {
    ...
    try {
      ...
      debugPrint("🔍 استخدام النظام الجديد للفلاتر: $currentFilterKey");
       ...
       try {
         final categoryFilterController = Get.find<EnhancedCategoryFilterController>();
    ...
    debugPrint("🔍 [ProductGridWidget _buildProductStream] Applying Sort: ${currentSortOption.label}, Filter: $currentFilterKey");
    debugPrint("🔍 فلتر النوع: $currentFilterKey");
    debugPrint("🔍 نوع النظام: ${currentFilterKey.contains('_') && currentFilterKey != 'all_items' ? 'نظام جديد (ID-based)' : 'نظام قديم (typeItem)'}");
    ...
    debugPrint("🔍 مقارنة الفلاتر: currentFilterKey='$currentFilterKey' vs allItemsFilterKey='$allItemsFilterKey'");
    if (currentFilterKey != allItemsFilterKey) {
      ...
      if (currentFilterKey.startsWith('barcode_')) {
        ...
        final productId = currentFilterKey.replaceFirst('original_product_', '');
        ...
        final companyId = currentFilterKey.replaceFirst('original_company_', '');
        debugPrint("🎯 تطبيق فلتر الشركة الأصلية: originalCompanyId=$companyId");
        query = query.where('itemCondition', isEqualTo: 'original')
        ...
        query = query.where('itemCondition', isEqualTo: 'original');
      } else if (currentFilterKey.startsWith('main_') || currentFilterKey.startsWith('sub_')) {
    ...
    if (!skipSort) {
      ...
      bool hasSpecificCategoryFilter = currentFilterKey.startsWith('main_') || 
      ...
      if (!hasSpecificCategoryFilter && currentFilterKey == allItemsFilterKey) {
        ...
        debugPrint("--> تطبيق الترتيب: ${currentSortOption.field}, descending: ${currentSortOption.descending}");
        try {
          query = query.orderBy(currentSortOption.field, descending: currentSortOption.descending);
    ...
    debugPrint("   - الترتيب: ${currentSortOption.field} (${currentSortOption.descending ? 'تنازلي' : 'تصاعدي'})");
  ...
  Future<void> _showAdminContextMenu(BuildContext context, TapDownDetails details, ItemModel item, GetStreamBuildBoxOfItemController controller) async {
    if (!_isAdmin) return;
      ...
      items: <PopupMenuEntry<String>>[
        ...
        PopupMenuItem<String>(
          ...
          child: ListTile(
            ...
            title: Text('إضافة كعرض', style: TextStyle(color: Colors.blue[700])), // <<-- تعريب
            dense: true,
            contentPadding: EdgeInsets.zero,
          ),
        ),
  ...
  void _showAddOfferDialog(BuildContext context, ItemModel item) {
    ...
    final TextEditingController rateController = TextEditingController(); // اختياري
    final Rxn<DateTime> expiryDate = Rxn<DateTime>(null); // لتاريخ الانتهاء (تفاعلي)
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              inputFormatters: [FilteringTextInputFormatter.digitsOnly], // السماح بالأرقام فقط
              validator: (value) {
                if (value == null || value.trim().isEmpty) { return 'سعر العرض مطلوب'; }
                final int? price = int.tryParse(value.trim());
                if (price == null || price <= 0) { return 'أدخل سعراً صحيحاً أكبر من صفر'; }
                if (price >= oldPrice) { return 'سعر العرض يجب أن يكون أقل من السعر الأصلي';} // تحقق إضافي
                return null;
              },
            ),
            ...
            Obx(() => ListTile( // استخدام Obx لتحديث النص عند اختيار تاريخ
              ...
              trailing: expiryDate.value != null
                  ? IconButton(
                icon: Icon(Icons.clear, size: 20),
                onPressed: () => expiryDate.value = null, // مسح التاريخ
              ) : null,
              onTap: () async {
                DateTime? pickedDate = await showDatePicker(
                  context: context,
                  initialDate: expiryDate.value ?? DateTime.now().add(const Duration(days: 7)), // الافتراضي بعد أسبوع
                  firstDate: DateTime.now(), // لا يمكن اختيار تاريخ في الماضي
                  lastDate: DateTime.now().add(const Duration(days: 365 * 2)), // سنتين كحد أقصى مثلاً
                );
                if (pickedDate != null) {
      ...
      confirm: ElevatedButton(
        ...
        onPressed: () {
          ...
          if (newPrice == null || newPrice <= 0 || newPrice >= oldPrice) {
            ...
            Get.snackbar("خطأ", "الرجاء إدخال سعر عرض صحيح أقل من السعر الأصلي.",
          ...
          final offerData = {
            'newPrice': newPrice,
            'oldPrice': oldPrice,
            'rate': int.tryParse(rateController.text.trim()) ?? 0, // استخدم 0 إذا كان فارغاً
  ...
  Future<void> _saveAsOffer(ItemModel originalItem, Map<String, dynamic> offerDetails) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance
          .collection(FirebaseX.offersCollection) // تأكد من أن هذا هو اسم مجموعة العروض الصحيح
          .doc(newOfferId)
          .set(newOffer.toMap());

      _showSnackbar('نجاح', 'تمت إضافة المنتج "${originalItem.name}" كعرض بنجاح.', Colors.green);
  ...
  void _showSnackbar(String title, String message, Color backgroundColor) {
    ...
    Get.snackbar( title, message, snackPosition: SnackPosition.BOTTOM, backgroundColor: backgroundColor, colorText: Colors.white, margin: const EdgeInsets.all(10), borderRadius: 8);
  }


  // دالة مساعدة لتحديد اللون بناءً على درجة الجودة
  Color _getQualityColor(int? grade) {
    if (grade == null) return Colors.grey.shade400; // لون افتراضي إذا لم تتوفر الدرجة
    if (grade >= 1 && grade <= 4) {
  ...
  Widget _buildEditTextField(TextEditingController controller, String label, TextInputType inputType) {
    return Padding(
  ...
  void _showDeleteConfirmationDialog(BuildContext context, String itemId) {
    final logic = Get.find<GetStreamBuildBoxOfItemController>(); // احصل على المتحكم

    Get.defaultDialog(
      title: "تأكيد الحذف", // <<-- تعريب
      titleStyle: TextStyle(fontWeight: FontWeight.bold, color: Colors.red[700]),
      middleText: "هل أنت متأكد من رغبتك في حذف هذا المنتج نهائيًا؟ لا يمكن التراجع عن هذا الإجراء.", // <<-- تعريب
      ...
      confirm: ElevatedButton.icon(
        ...
        onPressed: () async {
          ...
          try {
            await FirebaseFirestore.instance.collection(FirebaseX.itemsCollection).doc(itemId).delete();
            logic.showSnackbar('نجاح', 'تم حذف المنتج بنجاح.', Colors.green); // <<-- تعريب
          } catch (e) {
            logic.showSnackbar('خطأ', 'فشل حذف المنتج.', Colors.red); // <<-- تعريب
  ...
  Widget build(BuildContext context) {
    return GetBuilder<EnhancedCategoryFilterController>(
      ...
      builder: (controller) {
        ...
        final GetStreamBuildBoxOfItemController logic = Get.put(GetStreamBuildBoxOfItemController());
        Get.put(FavoriteController());

        debugPrint("🔄 إعادة بناء ProductGridWidget - الفلتر المطبق: ${controller.getFilterKey()}");

        return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          stream: _buildProductStream(), // <-- الدالة تستخدم الترتيب من Get.find الآن
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          ...
          if (snapshot.error.toString().contains('requires an index')) {
            ...
            debugPrint("🔧 خطأ الفهرس المكتشف - جاري إعادة المحاولة بدون ترتيب");
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return _buildShimmerLoadingGrid(context); // <-- استخدام دالة Shimmer
        }
        ...
        if (docs.isEmpty) {
          ...
          FirebaseFirestore.instance
              ...
              .get()
              .then((querySnapshot) {
                debugPrint("🔍 عدد المنتجات الإجمالي في التطبيق: ${querySnapshot.docs.length}");
                if (querySnapshot.docs.isNotEmpty) {
                  final sampleItem = querySnapshot.docs.first.data();
                  debugPrint("🔍 مثال على منتج في قاعدة البيانات:");
                  debugPrint("   - mainCategoryId: ${sampleItem['mainCategoryId']}");
                  debugPrint("   - subCategoryId: ${sampleItem['subCategoryId']}");
                  debugPrint("   - typeItem: ${sampleItem['typeItem']}");
                  debugPrint("   - itemCondition: ${sampleItem['itemCondition']}");
                }
              });
          
          return _buildEmptyStateWidget(context); // <-- استخدام دالة الحالة الفارغة المحسنة
        }
        // بناء الشبكة
        return Padding(
          ...
          child: GridView.builder(
            ...
            shrinkWrap: true, // لتناسب حجم المحتوى داخل الـ ListView الرئيسي
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 2, // عدد الأعمدة
              // تعديل نسبة العرض إلى الارتفاع لتناسب المحتوى بشكل أفضل
              childAspectRatio: (wi * 0.5) / (hi * 0.35), // زيادة الارتفاع النسبي قليلاً
            ...
            itemBuilder: (context, index) {
              try {
                ...
                return Card(
                    color: Colors.red[50],
                    child: Center(
                        child: Padding( padding: const EdgeInsets.all(8.0), child: Column( mainAxisSize: MainAxisSize.min, children: [ Icon(Icons.warning_amber_rounded, color: Colors.red[400], size: 30,), SizedBox(height:5), Text('خطأ في عرض المنتج', textAlign: TextAlign.center, style: TextStyle(color: Colors.red[700])), // <<-- تعريب
                        ]))
                    ));
              }
            },
  ...
  Widget _buildProductSkeletonCard(BuildContext context) {
    final wi = MediaQuery.of(context).size.width;
    final hi = MediaQuery.of(context).size.height;
    final cardHeight = hi * 0.35; // نفس ارتفاع البطاقة
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          // منطقة الصورة
          Expanded( flex: 3, child: Container(color: Colors.white)), // لون الخلفية للشيمر
          // منطقة النص والأزرار
          Expanded( flex: 2, child: Padding( padding: const EdgeInsets.fromLTRB(8, 8, 8, 4), child: Column( crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Container(width: cardWidth * 0.8, height: 12, color: Colors.white), // شريط لاسم المنتج
            SizedBox(height: 5),
            Container(width: cardWidth * 0.5, height: 10, color: Colors.white), // شريط للسعر/أزرار
          ]),),
          ),
        ],
      ),
  ...
  Widget _buildShimmerLoadingGrid(BuildContext context) {
    ...
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 7),
      child: Shimmer.fromColors( // <-- تغليف الشبكة بالشيمر
        ...
        child: GridView.builder(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 2,
            childAspectRatio: (wi * 0.5) / (hi * 0.35), // نفس النسبة
            crossAxisSpacing: 10,
            mainAxisSpacing: 10,
          ),
          itemCount: 6, // عرض 6 هياكل تحميل
          itemBuilder: (context, index) => _buildProductSkeletonCard(context), // بناء الهيكل
        ),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'لم نتمكن من تحميل المنتجات. الرجاء التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى.', // <<-- تعريب
              style: TextStyle(fontSize: 15, color: Colors.grey),
              textAlign: TextAlign.center,
            ),
            // يمكن إضافة زر إعادة محاولة هنا إذا لزم الأمر
            // SizedBox(height: 20),
            // ElevatedButton.icon(onPressed: (){ /* استدعاء دالة إعادة التحميل */}, icon: Icon(Icons.refresh), label: Text("إعادة المحاولة"))
          ],
        ),
      ),
    );
  ...
  Widget _buildEmptyStateWidget(BuildContext context) {
    ...
    try {
      ...
      isFiltered = filterController.hasActiveFilter.value;
       ...
       isFiltered = (widget.selectedSubtypeKey != null && widget.selectedSubtypeKey != allItemsFilterKey);
     }
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(
              isFiltered ? Icons.filter_alt_off_outlined : Icons.shelves, // أيقونة مختلفة للفلتر
              size: 70,
              color: Colors.grey[400],
            ),
            const SizedBox(height: 20),
            Text(
              isFiltered ? 'لا توجد منتجات تطابق هذا الفلتر' : 'لا توجد منتجات بعد!', // <<-- تعريب
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: Colors.grey[700]),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 10),
            Text(
              isFiltered ? 'جرب اختيار نوع آخر أو قم بإزالة الفلتر.' : 'كن أول من يرى جديدنا!', // <<-- تعريب
              style: TextStyle(fontSize: 15, color: Colors.grey[600]),
              textAlign: TextAlign.center,
            ),
  ...
  // --- بناء بطاقة المنتج الفردي ---
  Widget _buildItemCard(BuildContext context, ItemModel item, double wi, double hi, GetStreamBuildBoxOfItemController controller) {
    final theme = Theme.of(context); // الحصول على الثيم لاستخدامه
    final FavoriteController favoriteCtrl = Get.find<FavoriteController>(); // <-- مهم هنا

    // --- 1. تحضير نص الحالة والجودة واللون ---
    String conditionText = '';
    if (item.itemCondition == 'original') {
      conditionText = 'أصلي'; // تعريب الحالة
    } else if (item.itemCondition == 'commercial') {
      conditionText = 'تجاري';
    }
    // تحضير نص درجة الجودة "د.X"
    String qualityText = item.qualityGrade != null ? 'د.${item.qualityGrade}' : '';
    ...
    Color qualityColor = _getQualityColor(item.qualityGrade); // استدعاء الدالة المساعدة
    // -------------------------------------
    ...
    return Card(
      ...
      child: InkWell(
        ...
        onTapDown: (details) { if (_isAdmin) _showAdminContextMenu(context, details, item, controller); },
        onLongPress: () { if (_isAdmin) { final center = Offset(wi / 2, hi / 2); _showAdminContextMenu(context, TapDownDetails(globalPosition: center), item, controller); } },
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch, // لجعل العناصر الفرعية تملأ العرض
          children: [
            // --- 3. الجزء العلوي: الصورة مع الشريط ---
            Expanded(
              flex: 3, // أعطه مساحة أكبر من الجزء السفلي
              child: Stack(
                ...
                children: [
                  ...
                  if (item.videoUrl != null && item.videoUrl != 'noVideo')
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: const Icon(Icons.play_circle_fill, size: 20, color: Colors.white),
                      ),
                    ),

                  // --- شريط الحالة والجودة (يظهر فقط إذا كان هناك نص) ---
                  if (combinedStatusText.trim().isNotEmpty) // تحقق أن النص غير فارغ بعد إزالة المسافات
                    Positioned(
                      top: 6,  // المسافة من الحافة العلوية
                      right: 6, // المسافة من الحافة اليمنى
                      child: IgnorePointer( // لمنع هذا الشريط من اعتراض نقرات المستخدم على البطاقة
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), // تباعد داخلي حول النص
                          decoration: BoxDecoration(
                              color: qualityColor.withOpacity(0.9), // اللون المحسوب مع شفافية طفيفة
                              borderRadius: BorderRadius.circular(6), // حواف دائرية
                              // إضافة حدود خفيفة إذا أردت (اختياري)
                              // border: Border.all(color: Colors.white.withOpacity(0.5), width: 0.5),
                              boxShadow: [ // ظل خفيف لتحسين الوضوح (اختياري)
                                BoxShadow( color: Colors.black.withOpacity(0.25), blurRadius: 4, offset: Offset(0, 1), )
                              ]
                          ),
                          child: Text(
                            ...
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis, // للتعامل مع النص الطويل جدًا (غير محتمل هنا)
                          ),
                        ),
                      ),
                    ),
                  // ------------------------------------
                  Positioned(
                    top: combinedStatusText.trim().isNotEmpty ? 35 : 6, // <--- ضبط الموقع بناءً على وجود الشريط
                    right: 6,
                    // استخدام StreamBuilder لمراقبة حالة المفضلة الحالية
                    child: StreamBuilder<bool>(
                        stream: favoriteCtrl.isFavoriteStream(item.id), // الاستماع للمنتج الحالي
                        builder: (context, favSnapshot) {
                          ...
                          final bool isFavorite = favSnapshot.data ?? false; // افتراض أنه ليس مفضل

                          return Material( // استخدام Material لـ InkWell/splash effect
                            color: Colors.black.withOpacity(0.3), // خلفية شبه شفافة
                            ...
                            child: InkWell(
                              ...
                              child: Padding(
                                ...
                                child: Icon(
                                  ...
                                  color: isFavorite ? Colors.redAccent : Colors.white, // ألوان مميزة
            ...
            Expanded(
                flex: 2, // مساحة أقل نسبيًا
                child: Padding(
                  padding: const EdgeInsets.only(left: 8, right: 8, top: 6, bottom: 4), // تباعد متوازن
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.spaceBetween, // لتوزيع المساحة
                    children: [
                      // اسم المنتج
                      Text(
                        item.name,
                        style: theme.textTheme.titleSmall?.copyWith(
                          ...
                          fontSize: wi * 0.036, // حجم مناسب للبطاقة
                          height: 1.2, // تباعد أسطر أفضل إذا كان سطرين
                        ),
  ...
  void _showEditDialog(BuildContext context, GetStreamBuildBoxOfItemController controller, ItemModel item, {required bool isEditingName}) {
    // تهيئة المتحكم النصي بالقيمة الحالية
    final editController = TextEditingController(text: isEditingName ? item.name : item.price.toString());
    final String title = isEditingName ? "تعديل اسم المنتج" : "تعديل سعر المنتج";
    final String label = isEditingName ? "الاسم الجديد" : "السعر الجديد";
    final TextInputType keyboardType = isEditingName ? TextInputType.text : TextInputType.numberWithOptions(decimal: false); // استخدام لوحة أرقام للسعر
    ...
    Get.defaultDialog(
      ...
      content: Form(
        ...
        child: _buildEditTextField(editController, label, keyboardType), // استخدام الدالة لبناء حقل النص
      ),
      confirm: ElevatedButton(
        onPressed: () async {
          // تمرير القيمة الجديدة للمتحكم الذي يتولى التحديث في Firestore
          final controllerToUpdate = Get.find<GetStreamBuildBoxOfItemController>(); // العثور على المتحكم مجدداً للتأكد
          // قم بتحديث القيمة المحلية في المتحكم النصي المؤقت إذا أردت (ليس ضروريًا هنا)
          if(isEditingName) {
            controllerToUpdate.nameEditController.text = editController.text; // تحديث متحكم الاسم الرئيسي
            controllerToUpdate.isEditingName.value = true; // تحديد أننا نعدل الاسم
            controllerToUpdate.isEditingPrice.value = false;
          } else {
            controllerToUpdate.priceEditController.text = editController.text; // تحديث متحكم السعر الرئيسي
            controllerToUpdate.isEditingPrice.value = true; // تحديد أننا نعدل السعر
            controllerToUpdate.isEditingName.value = false;
          }
          Get.back(); // أغلق مربع الحوار الحالي
          await controllerToUpdate.confirmEdit(item.id); // استدعاء دالة التأكيد في المتحكم الرئيسي
        },
  ...
  Widget _buildItemImage(ItemModel item, double wi, double hi) {
    ...
    return Hero(
      ...
      child: CachedNetworkImage(
        ...
        errorWidget: (context, url, error) => Container(color: Colors.grey[100], child: const Icon(Icons.broken_image_outlined)),
