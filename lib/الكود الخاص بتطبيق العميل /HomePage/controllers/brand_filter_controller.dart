...
class BrandFilterController extends GetxController
  ...
  final RxList<CompanyProductModel> selectedCompanyProducts =
      <CompanyProductModel>[].obs;

  // الاختيارات الحالية
  final Rxn<CompanyModel> selectedCompany = Rxn<CompanyModel>();
  final Rxn<CompanyProductModel> selectedCompanyProduct =
      Rxn<CompanyProductModel>();
  ...
  Future<List<CompanyProductModel>> _loadProductsForCompany(
    String companyId,
  ) async {
    try {
      final QuerySnapshot snapshot =
  ...
  Future<void> activateBrandMode() async {
    ...
    debugPrint(
      '🏢 الشركة المختارة حالياً: ${selectedCompany.value?.nameAr ?? 'لا شيء'}',
    );
    debugPrint(
      '💾 آخر شركة محفوظة: ${_lastSelectedCompany?.nameAr ?? 'لا شيء'}',
    );
    ...
    if (companies.isEmpty) {
      ...
      debugPrint(
        '✅ عدد المنتجات المستعادة: ${_lastSelectedCompany!.products.length}',
      );
    }
  ...
  Future<void> deactivateBrandMode() async {
    ...
    debugPrint(
      '💾 حفظ الشركة المختارة: ${_lastSelectedCompany?.nameAr ?? 'لا شيء'}',
    );
    debugPrint(
      '💾 حفظ المنتج المختار: ${_lastSelectedCompanyProduct?.nameAr ?? 'لا شيء'}',
    );
  ...
  void clearAllMemoryData() {
    ...
    debugPrint(
      '🏢 الشركة المختارة: ${selectedCompany.value?.nameAr ?? 'لا شيء'}',
    );
    debugPrint(
      '💾 الشركة المحفوظة: ${_lastSelectedCompany?.nameAr ?? 'لا شيء'}',
    );
  ...
  void selectCompany(CompanyModel company) {
    ...
    debugPrint(
      '✅ تم تحديث الاختيار - الشركة المختارة: ${selectedCompany.value?.nameAr}',
    );
